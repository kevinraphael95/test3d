...
for (let i = 0; i < STAR_COUNT; i++) {
    const theta=2*Math.PI*Math.random(), phi=Math.acos(2*Math.random()-1), r=1600;
...
function setSky(s) {
    ...
    skyUniforms.horizonColor.value.copy(s.hor);
    skyUniforms.bottomColor.value.copy(s.bot);
}
function lerpSky(a, b, t) {
    skyUniforms.topColor.value.copy(a.top).lerp(b.top,t);
    skyUniforms.horizonColor.value.copy(a.hor).lerp(b.hor,t);
    skyUniforms.bottomColor.value.copy(a.bot).lerp(b.bot,t);
}
...
function updateDayNight(elapsed) {
    ...
    sunGlow.material.opacity    = Math.pow(sf,0.5)*0.8;
    moonSprite.material.opacity = Math.pow(mf,0.35);
    moonGlow.material.opacity   = Math.pow(mf,0.5)*0.7;
...
function initMusic() {
    ...
    audio.addEventListener('ended', ()=>setTimeout(play,120_000));
    let started=false;
...
function fbm(x,z) {
    return simplex2(x*0.002,z*0.002)*14+simplex2(x*0.008,z*0.008)*5+simplex2(x*0.025,z*0.025)*1.5;
}
...
function terrainNormal(wx,wz) {
    const d=HSTEP; return new THREE.Vector3(findY(wx-d,wz)-findY(wx+d,wz),2*d,findY(wx,wz-d)-findY(wx,wz+d)).normalize();
}
...
function _buildChunk(cx,cz,key) {
    ...
    for(let i=0;i<treeN;i++){
        ...
        do{wx=oX+(rng()-0.5)*CHUNK_SIZE*0.85;wz=oZ+(rng()-0.5)*CHUNK_SIZE*0.85;
           ok=!tpts.some(p=>{const dx=p[0]-wx,dz=p[1]-wz;return dx*dx+dz*dz<16*16;});}while(!ok&&++tries<15);
        tpts.push([wx,wz]);
        const gy=findY(wx,wz);
        const h=28+rng()*18, tr=1.4+rng()*1.0;
    ...
    for(let i=0;i<gn;i++){
        ...
        dummy.position.set(wx,findY(wx,wz)+0.25,wz); dummy.scale.setScalar(0.5+rng()*0.8); dummy.rotation.y=rng()*Math.PI; dummy.updateMatrix();
...
function unloadChunk(cx,cz){
    ...
    data.group.traverse(obj=>{
        if(!obj.isMesh)return;
        if(obj.geometry&&![GEO.grass,GEO.ff,GEO.stem,GEO.flower,GEO.rock].includes(obj.geometry))obj.geometry.dispose();
        const mats=Array.isArray(obj.material)?obj.material:[obj.material];
...
function updateMovement(dt){
    ...
    const norm=terrainNormal(camera.position.x,camera.position.z), slope=1-Math.abs(norm.y);
