...
function updateDayNight(elapsed) {
    ...
    else if (a < Math.PI*5/6) lerpSky(SKY.dawn,   SKY.day,    Math.min((a-Math.PI/6)/(Math.PI*2/3)*1.5, 1));
    ...
    else { skyUniforms.topColor.value.copy(SKY.night.top); skyUniforms.horizonColor.value.copy(SKY.night.horizon); skyUniforms.bottomColor.value.copy(SKY.night.bottom); }
}
...
function initMusic() {
    // Essayer le fichier MP3 d'abord
    ...
    audio.addEventListener('canplaythrough', () => { mp3Works = true; }, { once:true });
    audio.addEventListener('ended', () => {
        setTimeout(() => { audio.currentTime=0; audio.play().catch(()=>{}); }, 90000); // 90s silence
    });

    // Web Audio ambiance générative (vent + grillons) en parallèle ou fallback
    const actx = new (window.AudioContext || window.webkitAudioContext)();

    function makeWind() {
        // Bruit blanc filtré → vent doux
        ...
        src.connect(lp); lp.connect(gainNode); gainNode.connect(actx.destination);
        src.start();
        return gainNode;
    }

    function makeCrickets() {
        // Quelques oscillateurs haute fréquence → grillons
        const master = actx.createGain();
        master.gain.setValueAtTime(0, actx.currentTime);
        master.gain.linearRampToValueAtTime(0.04, actx.currentTime+5);
        ...
        for (let i = 0; i < 6; i++) {
            ...
            const ampOsc = actx.createOscillator();
            ampOsc.frequency.value = 8 + Math.random()*4;
            const ampGain = actx.createGain();
            ampGain.gain.value = 0;
            ampOsc.connect(ampGain.gain);
            const g = actx.createGain();
            g.gain.value = 0.015;
            osc.connect(g); g.connect(master);
            ampOsc.start(); osc.start();
        }
        return master;
    }

    function makeDroneNote(freq) {
        const osc = actx.createOscillator();
        osc.type = 'sine';
        osc.frequency.value = freq;
        const gain = actx.createGain();
        gain.gain.setValueAtTime(0, actx.currentTime);
        gain.gain.linearRampToValueAtTime(0.018, actx.currentTime+4);
    ...
    function startAmbiance() {
        ...
        makeWind();
        makeCrickets();
        // Accord ambiant très doux
        [110, 138.6, 165, 220].forEach(f => makeDroneNote(f));
...
function _buildChunk(cx,cz,key) {
    ...
    for(let i=0;i<treeCount;i++) {
        ...
        const h=16+rng()*18, tr=0.6+rng()*0.6;
        const trunkH=h*0.55;
        const tg=new THREE.Group();

        const trunk=new THREE.Mesh(new THREE.CylinderGeometry(tr*0.55,tr*1.1,trunkH+5,8),MAT.trunk);
        ...
        for(let li=0;li<layers;li++) {
            ...
            const cone=new THREE.Mesh(
                new THREE.ConeGeometry(tr*10*(1-ratio*0.72)+2.2, (h/layers)*2.0, 8),
                CONE_MATS[(rng()*3)|0]
            );
            cone.position.y=ratio*h*0.92;
    ...
    for(let i=0;i<rockCount;i++) {
        ...
        localColliders.push({type:'sphere',x:wx,y:gy+sy*0.5,z:wz,r:avgR,topY:gy+sy});
    }
    ...
    for(const c of globalColliders) {
        if(c.type==='cylinder') {
            ...
            if(dXZ<c.r+PLAYER_R&&ny>c.y&&pBot<cTop) {
                if(pBot>=cTop-0.6) { ny=cTop+PLAYER_H; onTop=true; }
                else { const a=Math.atan2(dz,dx); nx=c.x+Math.cos(a)*(c.r+PLAYER_R); nz=c.z+Math.sin(a)*(c.r+PLAYER_R); }
            }
        } else if(c.type==='sphere') {
            const dx=nx-c.x, dz=nz-c.z;
            const pMidY=ny-PLAYER_H*0.5;
            const dy=pMidY-c.y;
            const dist=Math.sqrt(dx*dx+dy*dy+dz*dz);
            const minD=c.r+PLAYER_R*1.4;
            if(dist<minD&&dist>0.001) {
                const pBot=ny-PLAYER_H;
                if(pBot>=c.topY-0.5&&dy>0) { ny=c.topY+PLAYER_H; onTop=true; }
                else {
                    const pen=minD-dist, inv=1/dist;
                    nx+=dx*inv*pen*0.85;
                    nz+=dz*inv*pen*0.85;
                }
            }
...
let camY=10; // Y lissé de la caméra
...
/* ===================================================== */
/* MOUVEMENT — Y lissé pour pentes fluides               */
/* ===================================================== */
...
function updateMovement(dt) {
    ...
    // Colliders XZ d'abord
    const res=resolveColliders(nx, camera.position.y, nz);
    nx=res.x; nz=res.z;

    // Sol cible
    const targetY=findY(nx,nz)+PLAYER_H;

    let ny;
    if(!grounded&&!res.onTop) {
        // En l'air : gravité pure, pas de lissage
        ny=camera.position.y+jumpVel;
        if(ny<=targetY){ ny=targetY; jumpVel=0; grounded=true; }
    } else {
        // Au sol ou sur objet : lisser le Y sur pentes
        // Lerp rapide (30 unités/s max) → jamais de saut brusque
        const targetFinal=res.onTop ? res.y : targetY;
        const diff=targetFinal-camY;
        // Vitesse de suivi du sol : rapide en descente, modérée en montée
        const followSpeed=diff<0 ? 20 : 10; // u/s
        camY+=Math.sign(diff)*Math.min(Math.abs(diff), followSpeed*dt);
        ny=camY;

        // Si on retombe sous le sol malgré le lerp, on force
        if(ny<targetY){ ny=targetY; camY=ny; }
...
// Démarrer à midi : dayAngle=PI/2 → elapsed = DAY_DURATION * (0.25)
// sin(PI/2)=1 donc plein jour
let elapsed=DAY_DURATION*0.25;

// Init camY au sol de départ
...
function animate() {
    ...
    for(const f of fireflyData){ f.mesh.position.y=f.baseY+Math.sin(t+f.phase)*0.5; f.mesh.position.x+=Math.cos(t*0.3+f.phase)*0.008; }
