...
function buildTower(wx, wz, grp, lc) {
    ...
    // ── GARDE-CORPS & COLLISIONS (RECTANGLES) ────────────
    const railTop = TOWER_H + 1.15;
    const gcSides = [
        // On a viré le côté Z+ (échelle)
    ...
    for (const s of gcSides) {
        ...
        // --- COLLISION RECTANGLE (Simple et robuste) ---
        lc.push({
            type: 'box',
            x: wx + s.cx,
            z: wz + s.cz,
    ...
    // ── PILIERS DU TOIT (Jusqu'à la plateforme) ──────────
    const roofPillarH = 5.5; // Plus grand pour atteindre le sol
    const roofY = TOWER_H + roofPillarH / 2; 
    for (const [px, pz] of [[-PLT_HALF, -PLT_HALF], [PLT_HALF, -PLT_HALF], [PLT_HALF, PLT_HALF], [-PLT_HALF, PLT_HALF]]) {
        ...
        // Positionné pour que le bas touche le plancher (TOWER_H)
    ...
    roofMesh.position.set(0, TOWER_H + roofPillarH + roofH * 0.5, 0);
    tg.add(roofMesh);
